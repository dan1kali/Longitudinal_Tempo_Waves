...
from longitudinal_tempo_waves.mixed_micromed_io.mixed_in_out import MicromedIO ############
...
class MicromedTRC(MicromedIO):
    ...
    def get_data(
        self,
        # picks: List = None,
        picks: Optional[List[str]] = None,  # <-- Optional makes None valid
        start: int = 0,
        # stop: int = None,
        stop: Optional[int] = None,         # <-- mark as Optional[int]
        keep_raw: bool = False,
        use_volt: bool = False,
    ):
        ...
        with open(self.filename, "rb") as f:
            ...
            self.decode_data_eeg_packet(bytearray(b_data), picks, keep_raw, use_volt)  # <-- convert to bytearray
