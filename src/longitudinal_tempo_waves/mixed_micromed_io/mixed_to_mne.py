...
from longitudinal_tempo_waves.mixed_micromed_io.mixed_trc import MicromedTRC


def create_mne_from_mixed_micromed_recording(
    recording_file: Union[str, Path],
    # sub_channels: List[str] = None,
    sub_channels: Optional[List[str]] = None,  # can be None
    start_time: float = 0.0,
    # stop_time: float = None,
    stop_time: Optional[float] = None,        # <-- mark as Optional[float]
    ...
    ch_types: list[str] = []
    for ch, ref in zip(sub_channels, micromed_trc.micromed_header.elec_refs):
        unit = ref.units

        if "dc" in ch.lower() or "png" in ch.lower():
            ch_types.append("misc")
        elif unit in ["nV", "μV", "uv", "mV", "V"]:
            if "emg" in ch.lower():
                ch_types.append("emg")
        ...
        elif "puls" in ch.lower() or "beat" in ch.lower() or "spo2" in ch.lower():
            ch_types.append("bio")
        else:
            ch_types.append("misc")
