...
class MicromedIO:
    ...
    def __init__(
        self,
        # picks: List[str] = None,
        picks: Optional[List[str]] = None,  # <-- use Optional

    ):
        self.micromed_header = MicromedHeader()
        # self.current_data_eeg = None
        self.current_data_eeg: np.ndarray = np.array([])
        self.picks = picks
        self.picks_id = None
    ...
    def decode_data_eeg_packet(
        ...
        packet: bytearray,
        # picks: List = None,
        picks: Optional[List[str]] = None,  # <-- allow None
        keep_raw: bool = False,
        use_volt: bool = False,
        ...
        # if n_bytes in [1, 2]:
        #     dt = np.dtype('<u2')  # unsigned 16-bit, little-endian
        # elif n_bytes == 4:
        #     dt = np.dtype('<i4')  # signed 32-bit, little-endian
        # else:
        #     raise ValueError(f"Error: code not yet implemented for n_bytes={n_bytes}.")
    ...
    def _check_eegs_data(self, mkr_channels) -> bool:
        ...
        """
        if self.current_data_eeg is None or self.current_data_eeg.size == 0 or not self.current_channels:
            logging.warning("EEG data or channels not initialized")
            return False
